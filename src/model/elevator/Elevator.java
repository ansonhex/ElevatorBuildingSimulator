...
public class Elevator implements ElevatorInterface {
  /************************************************************************
   * static value used to initialize the model.elevator id.
   ************************************************************************/
  private static int newElevatorId = 0;

  /************************************************************************
   * The id of the model.elevator.This is read only.
   ************************************************************************/
  private final int id = newElevatorId++;

  /************************************************************************
   * The total number of floors in the model.building.
   ************************************************************************/
  private final int maxFloor;

  /************************************************************************
   * The maximum number of people that can fit in the model.elevator.
   ************************************************************************/
  ...
  /************************************************************************
   * The number of steps that the model.elevator will wait at the top or bottom.
   ************************************************************************/
  private final int stopWaitTimeTotal = 5;

  /************************************************************************
   * The class variables that change as the model.elevator runs.
   ************************************************************************/
  private boolean takingRequests;


  /************************************************************************
   * The current floor of the model.elevator. Starts at 0.
   ************************************************************************/
  private int currentFloor;


  /************************************************************************
   * The direction the model.elevator is moving.
   ************************************************************************/
  ...
  private boolean outOfService;  // start must be issued on the model.elevator to start it.


  /**
   * The constructor for this model.elevator.
   * The model.elevator is initially at the ground floor and is not moving.
   *
   * @param maxFloor     the total number of floors in the model.building
   *                     must be greater than 0
   *                     must be less than 30 (city bylaws)
   * @param maxOccupancy the maximum number of people that can fit in the model.elevator
   *                     must be greater than 0
   ...
   * maxFloor getter.
   *
   * @return the total number of floors in the model.building
   */
  @Override
   ...
   * Direction getter.
   *
   * @return the direction the model.elevator is moving.
   */
  @Override
   ...
   *
   * start() - start the model.elevator
   * step() - move the model.elevator one step
   * processRequests() - process the requests
   * takeOutOfService() - take the model.elevator out of service
   * isTakingRequests() - is the model.elevator taking requests
   * toString() - string representation of the model.elevator
   * getElevatorStatus() - generate a report for the model.elevator in ElevatorReport format
   *
   * **********************************************************************/

  /**
   * Start the model.elevator.  This does nothing if the model.elevator is not
  ...
  /**
   * The step function is called to move the model.elevator one step.
   * First check to see if an out of service request has been made.
   * Second check to see if the door is open
   * Third Check to see if the model.elevator is waiting at the top or bottom.
   * Fourth check to see if there is a request at this floor.
   */
  public void step() {
    // If the model.elevator is out of service then we need to deal with that.
    ...
    // If the model.elevator is at the top or bottom we call the
    ...
    // we need to move the model.elevator in the direction it is currently moving.
  ...
  /**
   * Step the model.elevator when out of service.
   * If the model.elevator is on the ground floor and the door is open return
   * If the model.elevator is on the ground floor and the door is closed
   * then open the door.
   * <p></p>
   * If the model.elevator is not on the ground floor and the door is open
  ...
  private void stepOutOfService() {
    ...
    if (this.currentFloor == 0) {
      ...
      this.floorRequests[this.currentFloor] = false;
      // we can set the model.elevator to stopped.
      this.direction = Direction.STOPPED;
      return;
    }
    ...
    // set the direction of the model.elevator to down.
   ...
   * If a request is received to processRequests and the model.elevator
  ...
  /**
   * Take the model.elevator out of service.
   */
  @Override
  ...
  /**
   * isTakingRequests.
   * This will return true if the model.elevator is taking requests.
   *
  ...
  private void processStopRequests(List<Request> requests) {
    ...
    // if the model.elevator was waiting at the top or bottom
   ...
   *
   * @return string representation of the model.elevator.
   */
  @Override
  ...
  /**
   * Generate a report for the model.elevator in ElevatorReport format.
   *
  ...
  /**
   * Helper method to clear the static counter.
