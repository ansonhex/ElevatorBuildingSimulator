...
public class ElevatorTest {
  ...
  public void elevatorStartsAtGroundFloor() {
    ...
    model.elevator.Elevator elevator = new Elevator(10, 5);
  ...
  /**
   * Test the constructor for model.elevator id.
   */
  @Test
  ...
  /**
   * Test that the model.elevator starts out of service.
   */
  @Test
  ...
  /**
   * Test that the model.elevator reports stopped when at the top floor,
  ...
  public void elevatorReportsStopped() {
    ...
    // send the model.elevator to the top floor
    for (int i = 0; i < 3; i++) {
      elevator.step();
    }
    assertEquals(elevator.getDirection(), Direction.STOPPED);
    ...
    // send the model.elevator to the ground floor
    for (int i = 0; i < 3; i++) {
      elevator.step();
    }
    assertEquals(elevator.getDirection(), Direction.STOPPED);
    ...
    // now put the model.elevator out of service.
    elevator.takeOutOfService();
    // Do one step to let the model.elevator process the out of service request.
    elevator.step();
    assertFalse(elevator.isTakingRequests());
  ...
  /**
   * Test that the model.elevator can be taken out of service when it is not on the ground floor
   * and has no requests.
   */
  @Test
  public void elevatorOutOfService() {
    ...
    // send the model.elevator up three floors
    for (int i = 0; i < 3; i++) {
      elevator.step();
    }
  ...
  /**
   * Test that the model.elevator can process a request.
   */
  @Test
  ...
  public void elevatorProcessesRequests() {
    ...
    // The model.elevator has finished on the first floor and is now on the floor 1.
    // The door is closed.
    elevatorTenFloors10Capacity.step();
    ...
    // The model.elevator has finished on the second floor and is now on the floor 2.
    elevatorTenFloors10Capacity.step();
    ...
    // The model.elevator has finished on the third floor and is now on the floor 3.
    elevatorTenFloors10Capacity.step();
    ...
    // The model.elevator has finished on the fourth floor and is now on the floor 4.
    elevatorTenFloors10Capacity.step();
    ...
    // The model.elevator has finished on the fifth floor and is now on the floor 5.
    elevatorTenFloors10Capacity.step();
    ...
    // The model.elevator has finished on the sixth floor and is now on the floor 6.
    elevatorTenFloors10Capacity.step();
    ...
    // The model.elevator has finished on the seventh floor and is now on the floor 7.
    elevatorTenFloors10Capacity.step();
    // check that the floor is 6
    assertEquals(6, elevatorTenFloors10Capacity.getCurrentFloor());
    // check that the door is open
    ...
    // the model.elevator should now be going up, its door closed and on floor 0
  ...
  public void elevatorOnGroundFloorSentToTopFloor() {
    ...
    // model.elevator should be on floor 2 with the door closed.
    assertEquals(2, elevatorThreeFloors3Capacity.getCurrentFloor());

    // Now we follow the model.elevator down the model.building.
    elevatorThreeFloors3Capacity.step();

    // The model.elevator should be on floor 2 with the door opened.
    assertEquals(2, elevatorThreeFloors3Capacity.getCurrentFloor());
    ...
    // The model.elevator should be on floor 2 with the door closed.
    assertEquals(2, elevatorThreeFloors3Capacity.getCurrentFloor());
    assertTrue(elevatorThreeFloors3Capacity.isDoorClosed());
    elevatorThreeFloors3Capacity.step();

    // The model.elevator should be on floor 1 with the door closed.
    assertEquals(1, elevatorThreeFloors3Capacity.getCurrentFloor());
    assertTrue(elevatorThreeFloors3Capacity.isDoorClosed());
    elevatorThreeFloors3Capacity.step();

    // The model.elevator should be on floor 1 with the door opened.
    assertEquals(1, elevatorThreeFloors3Capacity.getCurrentFloor());
    assertFalse(elevatorThreeFloors3Capacity.isDoorClosed());
    elevatorThreeFloors3Capacity.step();
    ...
    // The model.elevator should be on floor 0 with the door closed.
    assertEquals(0, elevatorThreeFloors3Capacity.getCurrentFloor());
    assertTrue(elevatorThreeFloors3Capacity.isDoorClosed());
    elevatorThreeFloors3Capacity.step();

    // The model.elevator should be on floor 0 with the door opened.
    assertEquals(0, elevatorThreeFloors3Capacity.getCurrentFloor());
    assertFalse(elevatorThreeFloors3Capacity.isDoorClosed());
    elevatorThreeFloors3Capacity.step();
    ...
    // the model.elevator should be on floor 0 and waiting for 5 steps
    ...
    // The model.elevator has finished on the  floor 0 and is now on the floor 1.
    elevatorThreeFloors3Capacity.step();
    // check that the floor is 1
    assertEquals(1, elevatorThreeFloors3Capacity.getCurrentFloor());

    // The model.elevator has finished on the floor 1 and is now on the floor 2.
    elevatorThreeFloors3Capacity.step();
    // check that the floor is 2
    assertEquals(2, elevatorThreeFloors3Capacity.getCurrentFloor());
    elevatorThreeFloors3Capacity.step();

    // the model.elevator should be waiting for 5 steps
    assertEquals("Waiting[Floor 2, Time 5]", elevatorThreeFloors3Capacity.toString());

  }

  /**
   * test that an model.elevator midrun reports it is not taking requests
  ...
  public void elevatorThrowsExceptionWhenProcessCalledMidRun() {
    ...
    // model.elevator is on floor 5
    assertEquals(5, elevatorTenFloors10Capacity.getCurrentFloor());

    // model.elevator is not taking requests
    assertFalse(elevatorTenFloors10Capacity.isTakingRequests());

    // model.elevator should throw an exception
    elevatorTenFloors10Capacity.processRequests(upTenRequests);
  }
   ...
   * The model.elevator should immediately flush all of its stop requests and head to
   ...
   * The final state of the model.elevator should be [0|-|O 3]< -- -- -- -- -- -- -- -- -- -->
   */
  @Test
  public void takeElevatorOutOfServiceWhenDoorIsClosed() {
    ...
    // Now we put the model.elevator back in service
    elevatorTenFloors10Capacity.start();
    elevatorTenFloors10Capacity.processRequests(upTenRequests);

    elevatorTenFloors10Capacity.step();
    // model.elevator is on floor 0, direction is up and door is open for 3 steps
    assertEquals(0, elevatorTenFloors10Capacity.getCurrentFloor());
    assertEquals(Direction.UP, elevatorTenFloors10Capacity.getDirection());
    assertFalse(elevatorTenFloors10Capacity.isDoorClosed());
    ...
    // the model.elevator is on floor 0 and the door is open for 2 steps
  ...
  public void getElevatorStatus() {
    ...
    // The data in the report should match the state of the model.elevator
  ...
  /**
   * Make the model.elevator go up and down 100000 times.
   * This is a stress test to see if the model.elevator can handle a large number of requests.
   */
  @Test
  public void stressTest() {
    ...
    for (int i = 0; i < 100000; i++) {
      ...
      // check that the model.elevator is on the ground floor
      assertEquals(0, elevatorTenFloors10Capacity.getCurrentFloor());
      // check that the model.elevator is taking requests
      assertTrue(elevatorTenFloors10Capacity.isTakingRequests());

      // wait 5 steps for the model.elevator to start
      for (int j = 0; j < 5; j++) {
        elevatorTenFloors10Capacity.step();
      }
      ...
      // check to see if the model.elevator is taking requests
      assertTrue(elevatorTenFloors10Capacity.isTakingRequests());
      // check to see if the model.elevator is at the top
      assertEquals(9, elevatorTenFloors10Capacity.getCurrentFloor());

      // wait for 5 steps and see if the model.elevator is going down.
      for (int j = 0; j < 5; j++) {
        elevatorTenFloors10Capacity.step();
      }
      assertEquals(Direction.DOWN, elevatorTenFloors10Capacity.getDirection());
      ...
      // check to see if the model.elevator is taking requests
      assertTrue(elevatorTenFloors10Capacity.isTakingRequests());
      // check to see if the model.elevator is at the bottom
      assertEquals(0, elevatorTenFloors10Capacity.getCurrentFloor());
